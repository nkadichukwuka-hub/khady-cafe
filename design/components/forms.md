...
containing the form, wired to a server action. **Client Component.**
...
- The form `action` is the `reserveTable` server action, driven by
  `useActionState` → `[state, formAction, pending]` (React 19). Action
  signature is `(prevState, formData)`.
- Fields: name (`Field type="text"`), party size (`Select`), preferred date
  (`Field type="date"` with `min` today), preferred time (`Select` of fixed
  slots). Slots + party sizes + validation live in `lib/reservation.ts` so the
  form and the action can't drift.
- On an error return, the fields are wrapped in `<div key={state.attempt}>` so
  they remount and re-apply `defaultValue` from the returned values; each field
  shows `error={state.errors?.<name>}`.
...
- On success the dialog body swaps to a confirmation panel (the message + a
  ...
  this avoids the post-`await` `startTransition(setOpen(false))` dance.
- The verb carries through: trigger "Reserve a table" → confirmation
  "we've pencilled in a table…".

**No backend.** The action validates, `console.info`s the request server-side,
and returns the confirmation. Nothing is stored or emailed yet.
